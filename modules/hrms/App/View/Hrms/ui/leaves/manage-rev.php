...
<div id="navbox">
    ...
    <button class='active'>Revoke | <?= $co_rev ?></button>
...
<table class='rd-table'>
    ...
    ?>
            <tr>
                ...
                <td style="text-align: left;"><?= $x['revoke_reason'] ?></td>
                <td style="border-right:0px;text-align:left;"><?= $x['status'] ?></td>
                <td style="border-left:0px;text-align:right;">
                    <button class="button-18" onclick="leaveRevokeApprove('<?= $x['id'] ?>')" <?php if ($x['status_id'] == 1) echo 'disabled'; ?>>Approve</button>
                    <button class="button-18" onclick="leaveRevokeReject('<?= $x['id'] ?>')" <?php if ($x['status_id'] == 2) echo 'disabled'; ?>>Reject</button>
                </td>
            </tr>
    <?php
        endif;
...
<script>
    function leaveRevokeApprove(leaveId) {
        console.log('leaveRevokeApprove: ' + leaveId)

        var formData = new FormData()
        formData.append("a", "aec-hrms-api-leaveRevokeApprove")
    ...
    function leaveRevokeReject(leaveId) {
        console.log('leaveRevokeReject: ' + leaveId)

        var formData = new FormData()
        formData.append("a", "aec-hrms-api-leaveRevokeReject")
        formData.append('leaveId', leaveId)
